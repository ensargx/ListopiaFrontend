...
const ProfileStatistics: React.FC<ProfileStatisticsProps> = ({
    ...
    // --- Yeni: sayıları state olarak tutuyoruz ---
    ...
    const handleStatClick = async (
        ...
        try {
            ...
            if (type === "watched") {
                response = await fetchWatchedMoviesByUser(userUuid, {
                    pageNumber: 0,
                    pageSize,
                });
                // --- sayıyı güncelle ---
                setCounts((c) => ({ ...c, watched: response.totalElements }));
            } else if (type === "watchlist") {
                response = await fetchWatchlistMoviesByUser(userUuid, {
                    pageNumber: 0,
                    pageSize,
                });
                setCounts((c) => ({ ...c, wantToWatch: response.totalElements }));
            } else {
                response = await fetchLikedMovies(userUuid, 0, pageSize);
                setCounts((c) => ({ ...c, liked: response.totalElements }));
            }
            ...
            const turkish =
                type === "watched"
                    ...
                    : type === "watchlist"
                        ? "izlenecek"
                        : "beğenilen";
            setError(`${turkish} filmler yüklenemedi. Lütfen tekrar deneyin.`);
            setMovies([]);
            setTotalPages(0);
        } finally {
            setLoading(false);
    ...
    const loadPage = async (page: number) => {
        if (!activeList || page < 0 || (totalPages > 0 && page >= totalPages))
            return;
        ...
        try {
            ...
            console.error(`Error fetching page ${page} for ${activeList}:`, err);
            const turkish =
                activeList === "watched"
                    ? "izlenen"
                    : activeList === "watchlist"
                        ? "izlenecek"
                        : "beğenilen";
            setError(`${turkish} filmlerin ${page + 1}. sayfası yüklenemedi.`);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div style={containerStyle}>
            <div style={cardsContainerStyle}>
                ...
                <div
                    ...
                    onMouseOver={(e) =>
                        ((e.currentTarget.style as CSSProperties).transform = "scale(1.03)")
                    }
                    onMouseOut={(e) =>
                ...
                >
                    ...
                    <div style={countStyle}>{counts.watched}</div>
                ...
                >
                    ...
                    <div style={countStyle}>{counts.wantToWatch}</div>
                ...
                >
                    ...
                    <div style={countStyle}>{counts.liked}</div>
            ...
            {activeList && (
                <div style={movieListStyle}>
                    ...
                    {!loading && !error && movies.length === 0 && (
                        <div style={emptyMessageStyle}>No movies found in this list.</div>
                    )}
                    {!loading && !error && movies.length > 0 && (
                        <ProfileCardSlider
                            items={movies}
